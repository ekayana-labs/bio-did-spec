...
derived from Ed25519 public keys or, for assets a wallet creates and pays for,
from a program derived address bound to that wallet; DID documents are
materialized from accounts owned by the `bio-did-registry` program on the
Solana blockchain, which acts as the verifiable data registry. Every
syntactically valid key-based `did:bio` identifier resolves - identifiers
without on-chain state resolve to a deterministic *generative* DID document
containing the subject key itself, at zero cost.
...
The `idstring` MUST be the base58-btc encoding of a 32-byte **subject**, which
takes one of two forms:

- A **key subject**: an Ed25519 public key, the **subject key**. Any Ed25519
  keypair generated by the DID controller yields a valid `did:bio`
  identifier; no registration transaction is required for the identifier
  to exist and resolve (see [Section 6.2 Read](#62-read-resolve)).
- An **owned subject**: a program derived address computed by the registry
  ...
  for an owned subject: it has no generative document and exists only
  through the registry (Section 6.1). Owned subjects give research assets
  (datasets, papers, claims) identifiers that their creator controls and
  pays for without anyone holding a key per asset.

The two forms are distinguished by the curve test of Section 6.2 step 3: a
subject that decodes to a valid Ed25519 curve point is a key subject,
otherwise it is an owned subject. Resolvers MUST NOT infer an owned
subject's authority from the identifier; only the registry account states
who controls it.
...
- Within this method, a `method-specific-id` is unique because it is a
  deterministic encoding of 32 bytes - a point in the Ed25519 public-key
  space for a key subject, a program derived address for an owned subject -
  scoped by the network segment; two distinct subjects cannot encode to the
  same `idstring` (base58 is injective), and the two forms never overlap
  because a program derived address is never a curve point.
- An owned subject is unique per `(authority, nonce)` pair: the same nonce
  under two authorities derives two unrelated subjects, and the authority
  MUST sign `initialize_owned`, so nobody can register an owned subject in
...
- **Owned:** invoke `initialize_owned(nonce)` with accounts `[payer,
  authority, did_account, system_program]`. The program derives the owned
  subject of Section 4.2 from the signing `authority` and `nonce`, creates
  the account at PDA `["bio-did", subject]` (124 bytes) holding version 1,
  the `#default` protected verification method with the **authority's**
  Ed25519 key carrying all five relationships, and empty controller/service
  sets. The authority's signature is REQUIRED; the payer MAY be a different
  account (sponsored creation) and, as with `initialize`, gains no
  authority. Creation fails if the account already exists, and a
  deactivated owned DID can never be re-created (Section 6.4). From version
  1 on, an owned DID is updated exactly like a key-based one (Section 6.3);
  in particular the authority may rotate its key in and out of the document
  under the same protected-method and last-authority rules.
...
3. Base58-decode `idstring` to the 32-byte subject; if it does not decode
   to exactly 32 bytes, return `invalidDid`. Determine the subject form:
   a **key subject** if the bytes decompress to a valid Ed25519 curve point,
   an **owned subject** otherwise (Section 4.2).
...
6. If no account exists, the account has no data, or the account is not owned
   by the program: for a key subject return the **generative document**
   (Section 5.6) with `didDocumentMetadata` containing `versionId: "0"` and
   `deactivated: false`; for an owned subject return error `notFound`, since
   no keypair can stand behind such a subject and there is nothing to
   materialize.
...
data, so it fails the step 6 ownership check and resolves generatively (or
to `notFound` for an owned subject). A resolver's trust in a
...
operations - `initialize` or `initialize_owned` must be executed first.
...
significant read-path risk of this method; it does not apply to owned
subjects, for which withholding yields `notFound` rather than a document
anyone controls, so verifiers of owned DIDs see an outage, never a stale
authority. Mitigations: resolvers MUST
...
beyond donating rent), and `initialize_owned` can only create subjects the
signer itself derives; all caps (16 verification methods, 16 services, 8+8
...
- *Pre-registration compromise:* compromise of a subject key before
  ...
  critical. An owned subject has no pre-registration state: its authority
  is whatever key signed `initialize_owned`, and compromise of that key is
  the ordinary key-compromise case above.
...
- **Core library** (<https://github.com/ekayana-labs/did-bio-core>): the
  ...
  mapping, registry account deserialization, the key/owned subject test,
  owned subject derivation, and the generative fallback - with golden tests
  against this specification's examples.
...
flowchart TD
    ...
    C -->|not 32 bytes| E
    C --> D["Derive PDA: seeds 'bio-did' + subject,<br/>program H1gnV4Gj...3Xxy6"]
    D --> F["getAccountInfo at finalized commitment"]
    F -->|"no account, empty, or wrong owner<br/>and subject on the curve (key subject)"| G["Generative DID document<br/>versionId = 0, deactivated = false"]
    F -->|"no account, empty, or wrong owner<br/>and subject off the curve (owned subject)"| N["error: notFound"]
...
stateDiagram-v2
    ...
    [*] --> Registered : initialize_owned(nonce) - authority signs, subject derived
